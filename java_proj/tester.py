...
class testCase():
    ...
    def __init__(self, pid, planes, wait_time, wait_after, total_time, possible, expected_fuel,
            expected_land_1, expected_land_2):
        self.pid = pid
...
planes = [(1,10,1,1,1,2)]
wait_time = [0]
cases[0] = testCase(1, planes, wait_time, 0, 0, True, [8], [2], [1])

# wait in air
planes = [(1,10,1,1,1,1), (2,10,1,1,1,1), (3,10,1,1,1,1)]
wait_time = [0,0,0]
cases[1] = testCase(2, planes, wait_time, 0, 0, True, [8,8,7], [2,2,3], [1,1,1])

# no wait (long enough delay)
planes = [(1,10,1,1,1,4), (2,10,1,1,1,1), (3,10,1,1,1,1)]
wait_time = [0, 4.5, 4]
cases[2] = testCase(3, planes, wait_time, 0, 0, True, [8,8,8], [2,6,10], [1,5,9])

# no wait (very close delay)
planes = [(1,10,1,1,1,4), (2,10,1,1,1,5), (3,10,1,1,3,2)]
wait_time = [0, 1.5, 1]
cases[3] = testCase(4, planes, wait_time, 0, 0, True, [8,8,6], [2,3,10], [1,2,3])

# more in each time unit (beginning & end)
planes = [(1,10,1,1,1,1), (2,10,1,1,1,1)]
wait_time = [0, 0.5]
cases[4] = testCase(5, planes, wait_time, 0, 0, True, [8,8], [2,2], [1,1])

# fuel fraction success
planes = [(1,10,3,1,1,4)]
wait_time = [0]
cases[5] = testCase(6, planes, wait_time, 0, 0, True, [4], [2], [1])

# fuel fraction crash
planes = [(1,5,3,1,1,4), (2,10,1,1,1,1)]
wait_time = [0, 4.5]
cases[6] = testCase(7, planes, wait_time, 0, 0, False, [], None, None)

# fuel 0
planes = [(1,0,1,1,1,4)]
wait_time = [0]
cases[7] = testCase(8, planes, wait_time, 0, 0, False, [], None, None)

# parking crash
planes = [(1,41,2,1,5,3), (2,41,2,1,5,3), (3,90,1,1,1,10)]
wait_time = [0,0,0]
cases[8] = testCase(9, planes, wait_time, 0, 0, False, [], None, None)

# crash in air
planes = [(1,10,1,1,1,1), (2,10,1,1,1,1), (3,1,1,1,1,1)]
wait_time = []
cases[9] = testCase(10,planes, wait_time, 0, 0, False, [], None, None)
